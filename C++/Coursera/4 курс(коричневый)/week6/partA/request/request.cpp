...
namespace Request {
	namespace {
		...
		RequestHolder Parse(const Mode mode, string_view str) {
		...
		vector<RequestHolder> SplitRead(Mode mode, istream& in_stream) {
			...
			for (int i = 0; i < count && getline(in_stream, request_str); ++i) {
				if (auto request = Parse(mode, request_str)) {
	...
	Splited Read(istream& in_stream) {
		return { SplitRead(Mode::MODIFY, in_stream),
			SplitRead(Mode::READ, in_stream) };
	}
