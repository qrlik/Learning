...
public:
	...
	void Read(int user_id, int page_count) { // O(P)
		UpdatePageRange(user_page_counts_[user_id] + 1, page_count + 1); // O(P)
...
int main() {
	...
	ReadingManager manager; // O(U + P)

	int query_count;
	cin >> query_count;

	for (int query_id = 0; query_id < query_count; ++query_id) { // O(UP + Q)
		string query_type;
		cin >> query_type;
		int user_id;
		cin >> user_id;
		...
		if (query_type == "READ") {
			int page_count;
			cin >> page_count;
			manager.Read(user_id, page_count);							// O(UP)
		}
