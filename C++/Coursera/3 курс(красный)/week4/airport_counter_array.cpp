...
public:
	...
	template <typename TIterator> AirportCounter(TIterator begin, TIterator end) : airports({ 0 })
	{
		for (; begin != end; ++begin)
		{
			++airports[static_cast<int32_t>(*begin)]; // добавляем вылеты
		}
	}
	...
	{
		return airports[static_cast<int32_t>(airport)];
	}

	// добавить данный элемент
	void Insert(TAirport airport)
	{
		++airports[static_cast<int32_t>(airport)];
	}

	// удалить одно вхождение данного элемента
	void EraseOne(TAirport airport)
	{
		airports[static_cast<int32_t>(airport)] = (airports[static_cast<int32_t>(airport)] == 0) ? 0 : airports[static_cast<int32_t>(airport)] - 1;
	}

	// удалить все вхождения данного элемента
	void EraseAll(TAirport airport)
	{
		airports[static_cast<int32_t>(airport)] = 0;
	}
...
void TestManyConstructions() {
	...
	for (auto& x : airports) {
		LOG_DURATION("for (auto& x : airports)");
