...
{
	vector<pair<size_t, int>> docid_count(data.size()); // счетчик hitcount для всех документов <hitcount, docid>
	...
	for (string current_query; getline(query_input, current_query); ) // получили запрос
	{
		...
		for (size_t i = 0; i < relevant_size && docid_count[i].first != 0; ++i) // выводим до 5 самых релевантных документов
		{
