...
{
	vector<pair<size_t, size_t>> docid_count(data.size()); // счетчик hitcount для всех документов <docid, hitcount>
	...
	for (string current_query; getline(query_input, current_query); ) // получили запрос
	{
		...
		for (size_t i = 0; i < relevant_size && docid_count[i].second != 0; ++i) // выводим до 5 самых релевантных документов
		{
		...
		docid_count.assign(data.size(), make_pair(0, 0));
	}
}
